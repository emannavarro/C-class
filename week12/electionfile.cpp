/**
    CS-11 Asn 12, electionsfile.cpp
    Purpose: improved hw
...
void loadData(vector<Candidate>& list, string fileName);
void saveData(const vector<Candidate>& list, string fileName);
void writeReport(vector<Candidate> list, string fileName);
...
int main() {
    ...
    while( input!=0) {
        ...
        cout<<"99.writeReport"<<endl;
        cout<<"Choice (0-5) or 99:";
        cin>>input;
        cout<< endl;

        if(input==0) {

            saveData(Can,"candidate.txt");
            return 0;
        } else if(input==1) {
            listcandidates(Can);
        } else if(input==2) {
...
void Candidate::read(ifstream& fin)
{
...
void Candidate::write(ofstream& fout) const {
